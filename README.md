...
-- ======================================================================
-- SMART BOOKING CAFÉ – FULL SCHEMA & SEED (3 ROLES: ADMIN / MANAGER / STAFF)
-- SQL Server (T-SQL) | Safe re-runnable
-- ======================================================================
...
CREATE TABLE app.app_user (
    ...
    password_hash   VARCHAR(200) NOT NULL, -- store a real hash in production
    is_active       BIT NOT NULL DEFAULT 1,
    created_at_utc  DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME()
);
...
CREATE TABLE app.recipe_bom (
    sku_id          INT NOT NULL,     -- finished SKU
    ingredient_id   INT NOT NULL,     -- ingredient SKU
...
CREATE TABLE app.stock_on_hand (
    branch_id   INT NOT NULL,
    sku_id      INT NOT NULL,
...
CREATE TABLE app.inventory_txn (
    ...
    reason      VARCHAR(30) NOT NULL, -- ORDER_CONSUME/GRN/ADJUST/WASTE/TRANSFER_OUT/TRANSFER_IN/VOID
...
CREATE TABLE app.waste_ticket (
    ...
    status    VARCHAR(20) NOT NULL DEFAULT 'OPEN', -- OPEN/APPROVED/VOID
    created_by INT NOT NULL,
    approved_by INT NULL,
    created_at_utc DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME()
);
CREATE TABLE app.waste_line (
    line_id INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE app.payment_method (
    method_code VARCHAR(20) PRIMARY KEY, -- CASH/CARD/MOMO/VNPAY
    name        NVARCHAR(100) NOT NULL
);
...
CREATE TABLE app.employee_shift (
    ...
    branch_id INT NOT NULL,
    user_id  INT NOT NULL,
    role_code VARCHAR(50) NOT NULL, -- ADMIN/MANAGER/STAFF (text label for scheduling)
...
-- Users (5 accounts; map to 3 roles)
...
-- MANAGER: POS_OPEN, POS_PAY, ORDER_VOID, VIEW_REPORT, INVENTORY_EDIT, PO_APPROVE
INSERT INTO app.role_permission(role_id, perm_id)
...
-- STAFF: POS_OPEN, POS_PAY, VIEW_REPORT
INSERT INTO app.role_permission(role_id, perm_id)
SELECT r.role_id, p.perm_id FROM app.role r
JOIN app.permission p ON p.code IN ('POS_OPEN','POS_PAY','VIEW_REPORT')
WHERE r.code='STAFF';

-- User-Role mapping (admin, manager, 3 staff)
...
INSERT INTO app.user_role(user_id, role_id)
SELECT u.user_id, r.role_id FROM app.app_user u JOIN app.role r ON u.username IN ('cashier','barista','waiter') AND r.code='STAFF';

-- Membership tiers & customers
INSERT INTO app.membership_tier(tier_code, name, min_points) VALUES
('SILVER', N'Silver', 0),
('GOLD',   N'Gold',   500),
('PLAT',   N'Platinum', 1500);

INSERT INTO app.customer(phone, name, email, tier_code) VALUES
...
INSERT INTO app.stock_on_hand(branch_id, sku_id, qty)
SELECT 1, sku_id, 1000 FROM app.sku WHERE is_ingredient=1;
INSERT INTO app.stock_on_hand(branch_id, sku_id, qty)
SELECT 2, sku_id, 800 FROM app.sku WHERE is_ingredient=1;

-- Suppliers + PO/GRN
INSERT INTO app.supplier(code, name, contact, payment_term) VALUES
('SUP01', N'Nhà cung cấp A', N'0938 000 111', N'Net 15'),
...
UNION ALL
SELECT 1, s.sku_id, 400, 2450, 'LOT-TEA-01', DATEADD(day,180,GETUTCDATE()) FROM app.sku s WHERE s.code='ING_TEA';

-- Inventory transactions from GRN
...
INSERT INTO app.stock_transfer_line(st_id, sku_id, qty)
SELECT 1, s.sku_id, 100 FROM app.sku s WHERE s.code='ING_TEA';
INSERT INTO app.inventory_txn(branch_id, sku_id, qty_change, reason, ref_type, ref_id)
...
INSERT INTO app.waste_line(ticket_id, sku_id, qty)
SELECT 1, s.sku_id, 50 FROM app.sku s WHERE s.code='ING_MILK';
INSERT INTO app.inventory_txn(branch_id, sku_id, qty_change, reason, ref_type, ref_id)
SELECT 1, s.sku_id, -50, 'WASTE','WT','1' FROM app.sku WHERE code='ING_MILK';
...
-- Loyalty seed
INSERT INTO app.points_ledger(customer_id, points, reason, ref_type, ref_id) VALUES
(1, 120, N'Welcome bonus', 'SYS','WBONUS');
...
FROM app.sku s1
JOIN app.recipe_bom rb ON rb.sku_id = s1.sku_id
JOIN app.sku s2 ON s2.sku_id = rb.ingredient_id
WHERE s1.code IN ('CF_LATTE','TEA_PEACH');

-- Bill (service 5%, VAT 8% demo)
DECLARE @subtotal DECIMAL(12,2) = (SELECT SUM(qty * unit_price) FROM app.order_item WHERE order_id=@order_id);
DECLARE @discount DECIMAL(12,2) = ROUND(@subtotal * 0.10, 0); -- apply WELCOME10 demo
...
/* ---------- HR sample ---------- */
-- Shift for STAFF at branch 1 (cashier user)
INSERT INTO app.employee_shift(branch_id, user_id, role_code, start_at_utc)
SELECT 1, u.user_id, 'STAFF', DATEADD(hour,-2,SYSUTCDATETIME()) FROM app.app_user u WHERE username='cashier';
...
-- Tip allocation to waiter (also STAFF)
INSERT INTO app.tip_ledger(shift_id, user_id, amount, source)
SELECT s.shift_id, u.user_id, 50000, 'POOL' FROM app.employee_shift s
JOIN app.app_user u ON u.username='waiter' WHERE s.role_code='STAFF';

/* ---------- Devices ---------- */
INSERT INTO app.device_printer(branch_id, type, ip) VALUES
(1, 'KDS', '192.168.1.101'),
(1, 'TICKET', '192.168.1.102'),
(1, 'BAR', '192.168.1.103');
